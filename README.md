# AnimeWidget

A simple Android home screen widget that shows your upcoming anime episodes.

Pulls your **Currently Watching** and **Plan to Watch** lists from [MyAnimeList](https://myanimelist.net), then fetches airing schedules from [AniList](https://anilist.co) to show what's airing next and when.

**Features:**
- Sorted by next airing time
- Tapping an entry opens it in [MoeList](https://github.com/axiel7/MoeList) (if installed) or MAL
- Manual refresh button
- Resizable widget
- Offline cache so the widget doesn't go blank between refreshes
- Option to toggle English/Romaji titles
- Option to include or exclude Plan to Watch
